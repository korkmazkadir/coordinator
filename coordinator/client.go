...
func (c *Client) AddNode(nodeAddress string) int {
	var count int
	err := c.client.Call("NodeCoordinator.AddNode", nodeAddress, &count)
	if err != nil {
		panic(err)
	}
	return count
...
func (c *Client) GetNodeCount(nodeAddress string) int {
	var count int
	err := c.client.Call("NodeCoordinator.GetNodeCount", nodeAddress, &count)
	if err != nil {
		panic(err)
	}
	return count
...
func (c *Client) GetNodeList(nodeAddress string) []string {
	var nodeList []string
	err := c.client.Call("NodeCoordinator.GetNodeList", nodeAddress, &nodeList)
	if err != nil {
		panic(err)
	}
	return nodeList
